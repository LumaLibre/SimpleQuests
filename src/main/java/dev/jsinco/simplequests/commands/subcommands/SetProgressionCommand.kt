...
class SetProgressionCommand : SubCommand {
    override fun execute(plugin: SimpleQuests, sender: CommandSender, args: Array<out String>) {
        if (args.size < 3) {
            sender.sendMessage("${Util.prefix}Usage: /simplequests setprogress <player> <category:questId> <progress>")
            return
        }
        ...
        return when (args.size) {
            ...
            3 -> {
                ...
                return questPlayer.completedQuests.also { it ->
                    it.addAll(questPlayer.activeQuests.map { it.fullIdentifier() })
                }
            }
            4 -> listOf("<progress>")
            else -> null
    ...
    override fun permission(): String? {
        return "simplequests.command.setprogress"
    }
